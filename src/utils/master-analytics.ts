...
export const trackMasterFormSubmission = (formType: string): void => {
  if (masterYandexId && typeof window !== 'undefined' && window.ym) {
    window.ym(parseInt(masterYandexId), 'reachGoal', 'form_submit', { form_type: formType });
  }
};

export const trackMasterButtonClick = (buttonName: string): void => {
  if (masterYandexId && typeof window !== 'undefined' && window.ym) {
    window.ym(parseInt(masterYandexId), 'reachGoal', 'button_click', { button_name: buttonName });
  }
};

export const trackMasterBooking = (serviceId: string): void => {
  if (masterYandexId && typeof window !== 'undefined' && window.ym) {
    window.ym(parseInt(masterYandexId), 'reachGoal', 'booking_started', { service_id: serviceId });
  }
};

export const trackMasterContact = (contactType: string): void => {
  if (masterYandexId && typeof window !== 'undefined' && window.ym) {
    window.ym(parseInt(masterYandexId), 'reachGoal', 'contact_click', { contact_type: contactType });
  }
};
